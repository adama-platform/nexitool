...
// (assumes quaternion is normalized; efficient and avoids pure quaternion math issues)
  ...
  public static void process(String inputPath, String outputPath, double y_center_ideal) throws IOException {
    ...
    double dy = big.min_y + y_center_ideal;

    System.out.println("COMPUTED:" + cy + "/" + dy);

    translateNodes(root, cx, dy, cz);
    System.out.println("[center-xy] " + Math.round(cx * 10) / 10.0 + ", " + Math.round(cz * 10) / 10.0);
    System.out.println("[y-range] ideal:" + y_center_ideal + " from: " + big.min_y + " to " + big.max_y);
